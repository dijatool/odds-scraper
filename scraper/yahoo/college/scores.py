...
def printScores( away, home, scoresArray ) :
	...
	if len( scoresArray ) < 5 :
		print "%18s   %s" % ( away, scoresArray[ 1 ] )
		print "%18s   %s" % ( home, scoresArray[ 2 ] )
	else :
		# the college scores leave a column out of the first row, fun
		chunkSize = 1 + ( len( scoresArray ) / 3 )
		offset = chunkSize - 1
...
def main() :
	...
	#url = "http://rivals.yahoo.com/ncaa/football/scoreboard?&w=7&y=2012&c=top25"
